...
#             base_slug = slugify(f"{instance.title}-{instance.created.strftime('%Y%m%d%H%M%S')}")
#         else:
#             return # do not change slug if title didn't change
        
#     else:
#         # It's a new article
#         base_slug = slugify(f"{instance.title}-{instance.created.strftime('%Y%m%d%H%M%S') if instance.created else ''}")
